...
func NewRealBQClient(ctx context.Context, projectID, env string) (*RealBQClient, error) {
	...
	datasetID := fmt.Sprintf("users_analytics_%s", env)
	return &RealBQClient{
		client:    client,
		projectID: projectID,
		env:       env,
...
func (b *RealBQClient) EnsureDatasetAndTables(ctx context.Context) error {
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = dataset.Create(ctx, &bigquery.DatasetMetadata{
				Location:    location,
				Description: "CDC Analytics dataset for Users boundary",
			})
			if err != nil {
	...
	// 2. Ensure users_cdc Table exists
	tableName := "users_cdc"
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = table.Create(ctx, &bigquery.TableMetadata{
				Schema:      schema,
				Description: "CDC raw changelog for collection users",
...
func (b *RealBQClient) InsertRow(ctx context.Context, row *ChangelogRow) error {
	table := b.client.Dataset(b.datasetID).Table("users_cdc")
