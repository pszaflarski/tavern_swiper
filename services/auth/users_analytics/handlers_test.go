...
func TestHandleUsersEvent_Insert(t *testing.T) {
	...
	mockBQ.On("InsertRow", mock.Anything, mock.MatchedBy(func(row *ChangelogRow) bool {
		assert.Equal(t, "user_123", row.DocumentID)
		assert.Equal(t, "INSERT", row.Operation)
		
		var dataMap map[string]interface{}
		err := json.Unmarshal([]byte(row.Data), &dataMap)
		assert.NoError(t, err)
		assert.Equal(t, "cdc-user@example.com", dataMap["email"])
	...
	req.Header.Set("ce-source", "//firestore.googleapis.com/projects/tavern-swiper-dev/databases/users-dev")
...
func TestHandleUsersEvent_Delete(t *testing.T) {
	gin.SetMode(gin.TestMode)

	// Create delete event data (only OldValue is present)
	docData := &firestoredata.DocumentEventData{
		OldValue: &firestoredata.Document{
			Name: "projects/tavern-swiper-dev/databases/users-dev/documents/users/user_999",
			Fields: map[string]*firestoredata.Value{
				"email": {
					ValueType: &firestoredata.Value_StringValue{
						StringValue: "deleted-user@example.com",
					},
				},
			},
			CreateTime: timestamppb.Now(),
	...
	mockBQ := new(MockBQClient)
	mockBQ.On("InsertRow", mock.Anything, mock.MatchedBy(func(row *ChangelogRow) bool {
		assert.Equal(t, "user_999", row.DocumentID)
		...
		assert.NoError(t, err)
		assert.Equal(t, "deleted-user@example.com", oldDataMap["email"])
