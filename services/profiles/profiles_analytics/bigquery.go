...
func NewRealBQClient(ctx context.Context, projectID, env string) (*RealBQClient, error) {
	...
	datasetID := fmt.Sprintf("profiles_analytics_%s", env)
	return &RealBQClient{
		client:    client,
		projectID: projectID,
		env:       env,
...
func (b *RealBQClient) EnsureDatasetAndTables(ctx context.Context) error {
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = dataset.Create(ctx, &bigquery.DatasetMetadata{
				Location:    location,
				Description: "CDC Analytics dataset for Profiles boundary",
			})
			if err != nil {
	...
	// 2. Ensure profiles_cdc Table exists
	tableName := "profiles_cdc"
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = table.Create(ctx, &bigquery.TableMetadata{
				Schema:      schema,
				Description: "CDC raw changelog for collection profiles",
...
func (b *RealBQClient) InsertRow(ctx context.Context, row *ChangelogRow) error {
	table := b.client.Dataset(b.datasetID).Table("profiles_cdc")
