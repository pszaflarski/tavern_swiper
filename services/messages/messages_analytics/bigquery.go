...
func NewRealBQClient(ctx context.Context, projectID, env string) (*RealBQClient, error) {
	...
	datasetID := fmt.Sprintf("messages_analytics_%s", env)
	return &RealBQClient{
		client:    client,
		projectID: projectID,
		env:       env,
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = dataset.Create(ctx, &bigquery.DatasetMetadata{
				Location:    location,
				Description: "CDC Analytics dataset for Messages boundary",
			})
			if err != nil {
	...
	if err != nil {
		if hasErrorCode(err, http.StatusNotFound) {
			...
			err = table.Create(ctx, &bigquery.TableMetadata{
				Schema:      schema,
				Description: "CDC raw changelog for collection discovery_matches_cache",
