...
type FirestoreClient interface {
	...
	DeleteCollection(ctx context.Context, col FirestoreCollection, batchSize int) error
	Close() error
}
...
func (r *RealFirestoreClient) DeleteCollection(ctx context.Context, col FirestoreCollection, batchSize int) error {
	realCol := col.(*RealFirestoreCollection).coll
	
	for {
		iter := realCol.Limit(batchSize).Documents(ctx)
		numDeleted := 0
...
var getDBFunc = func(ctx context.Context) (FirestoreClient, error) {
	...
	dbID := getEnv("FIRESTORE_DATABASE_ID", "bots-dev")
