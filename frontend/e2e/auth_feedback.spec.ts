...
test.describe('Auth Feedback and Toggle', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Structural Sentinel: Wait for the Auth screen container to mount
    await expect(page.getByTestId('auth-screen')).toBeVisible({ timeout: 15000 });
  });
  ...
  test('shows error message on failed login', async ({ page }) => {
    ...
    await page.getByPlaceholder('••••••••', { exact: true }).first().fill('wrong_password');
    await page.getByRole('button', { name: /Enter Tavern/i }).first().click();

    const errorText = page.getByTestId('auth-error-text').first();
  ...
  test('toggles password visibility', async ({ page }) => {
    const passwordInput = page.getByPlaceholder('••••••••', { exact: true }).first();
    const toggleButton = page.getByTestId('auth-password-toggle').first();
  ...
  test('clears error message when user starts typing', async ({ page }) => {
    const emailInput = page.getByPlaceholder('hero@realm.com', { exact: true }).first();
    const passwordInput = page.getByPlaceholder('••••••••', { exact: true }).first();
    const submitBtn = page.getByRole('button', { name: /Enter Tavern/i }).first();
